...
export default class GirouetteProvider {
  ...
  async #writeDecoratorCache(hash: string): Promise<void> {
    const cacheDir = join(this.app.appRoot.pathname, 'node_modules', '.cache', 'girouette')
    const cacheFilePath = join(cacheDir, 'decorators.cache')
    ...
    try {
      ...
      console.log(`[Girouette DEBUG] Cache directory ensured: ${cacheDir}`)
  ...
  /**
   * Generates routes registry and start/routes.ts file
   */
  async #generateRoutesFile() {
    const girouetteDir = join(this.app.appRoot.pathname, '.girouette')
    const startDir = join(this.app.appRoot.pathname, 'start')
    const cacheDir = join(this.app.appRoot.pathname, 'node_modules', '.cache', 'girouette')
    const registryFilePath = join(girouetteDir, 'routes_registry.ts')
    const routesFilePath = join(startDir, 'routes.ts')
    const cacheFilePath = join(cacheDir, 'routes.cache')

    // Create directories if they don't exist
    await mkdir(girouetteDir, { recursive: true })
    ...
    this.#logger?.info('[Girouette] ✗ Routes changed, regenerating routes registry')

    // Generate the routes registry file content
    const registryContent = `/*
|--------------------------------------------------------------------------
| Girouette Routes Registry
|--------------------------------------------------------------------------
| Auto-generated by Girouette - DO NOT EDIT
|
| This file contains all route definitions from decorated controllers.
| It is imported by start/routes.ts
|
*/
...
`
    ...
    // Generate the start/routes.ts file that imports the registry
...
import '../.girouette/routes_registry.js'
`

    // Write the files and cache
    this.#logger?.debug(`[Girouette] Writing routes registry to: ${registryFilePath}`)
    await Promise.all([
      writeFile(registryFilePath, registryContent, 'utf-8'),
      writeFile(routesFilePath, routesFileContent, 'utf-8'),
      writeFile(cacheFilePath, currentHash, 'utf-8'),
    ])

    this.#logger?.info('[Girouette] Routes changed, regenerated routes registry')
  }
