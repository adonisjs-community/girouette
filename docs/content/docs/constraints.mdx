...
```typescript
@Get('/users/:username')
@Where('username', async (value: string) => {
  // Could check against a blocklist
...
Here's a reference of commonly used patterns:
...
- Regex constraints are evaluated before the controller method is called
- Simple patterns are faster than complex ones
- Function matchers add overhead—use regex when possible
- Async matchers should be used sparingly
