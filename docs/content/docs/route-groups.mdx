...
export default class UsersV2Controller {
  ...
  async index() {
    ...
    return {
      version: 2,
      data: await User.all(),
      meta: { total: await User.query().count() }
    }
  }
}
```
...
- [Middleware](/docs/middleware) - Apply middleware to individual routes
- [Resources](/docs/resources) - RESTful resource controllers
- [Constraints](/docs/constraints) - Validate route parameters
